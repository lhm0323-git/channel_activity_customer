...
function customerLineClaimLink(claimToken) {
  const liffId = import.meta.env.VITE_LIFF_ID;
  if (!liffId || !claimToken) return "";
  return `https://liff.line.me/${liffId}?view=my-bookings&claimToken=${encodeURIComponent(claimToken)}`;
}
...
------------------------------------------
  ...
  const handleLoadMyBookings = async () => {
    try {
      ...
      if (claimBooking || claimToken) {
        ...
        await claimBookingWithLine(claimToken, lineProfile.accessToken, claimBooking);
        const url = new URL(window.location.href);
  ...
  const handlePrintSelectedClaimQrs = async () => {
    ...
    try {
      ...
      const failed = allRows.filter((row) => row.error);
  ...
  const handleImportBookingCsv = async (event) => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        payload.booking.employeeNumber = row.employeeNumber || "";
        try {
          const saved = await saveBooking(payload, { lineAccessToken: "", requireStaffSession: true });
          results.push({ rowNumber: row.rowNumber, name: row.name, bookingId: saved.bookingId, success: true, message: "已建立" });
        } catch (error) {
      ...
      const importedRows = results.filter((result) => result.success && result.bookingId);
      if (importedRows.length) setSelectedAdminBookingIds(importedRows.map((result) => result.bookingId));
