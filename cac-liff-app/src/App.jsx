...
const App = () => {
  ...
  const selectPublicPackage = (card) => {
    ...
    setBookingField("channel", audienceToChannel(card.tags.audience));
...
------------------------------------------
  ...
  const handleSaveAdminBooking = async () => {
    ...
    try {
      ...
      let fields = {
        ...
        medicalRecordNumber: adminDetailBooking.medicalRecordNumber || "",
  ...
  const handleBatchSendClaimEmails = async () => {
    const targets = selectedAdminBookings.filter((booking) => !booking.lineUserId && booking.customerEmail && booking.customerClaimToken);
    if (!targets.length) { setAdminStatus(lang === "en" ? "Select unlinked bookings with email first" : "請勾選具 Email 且尚未綁定 LINE 的預約"); return; }
    if (targets.length > 30) { setAdminStatus(lang === "en" ? "Send linking emails in groups of 30 or fewer" : "LINE 綁定信每次最多 30 筆，請分批寄送"); return; }
    if (!window.confirm(lang === "en" ? `Send ${targets.length} LINE linking emails? Delivery is throttled.` : `確定寄出 ${targets.length} 封 LINE 綁定信？系統將逐封限速寄送。`)) return;
    setAdminStatus(lang === "en" ? `Sending ${targets.length} linking emails...` : `正在寄送 ${targets.length} 封 LINE 綁定信...`);
    try {
      const result = (await sendBookingClaimEmails(targets.map((booking) => booking.bookingId))).data;
      setAdminStatus(lang === "en" ? `Linking email: ${result.sent.length} sent, ${result.skipped.length} skipped, ${result.failed.length} failed` : `LINE 綁定信：已寄 ${result.sent.length} 封、略過 ${result.skipped.length} 封、失敗 ${result.failed.length} 封`);
      handleLoadAdminBookings();
    } catch (error) { setAdminStatus(lang === "en" ? `Linking email failed: ${error.message}` : `LINE 綁定信寄送失敗：${error.message}`); }
  };
      ...
      <div className="w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="max-h-[70vh] overflow-y-auto p-4 space-y-4 text-sm">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <label className="text-xs font-bold text-slate-600">{t.phone}<input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm font-normal" value={adminDetailBooking.customerPhone || adminDetailBooking.phone || ""} onChange={(e) => setAdminDetailBooking({ ...adminDetailBooking, customerPhone: e.target.value })} /></label><label className="text-xs font-bold text-slate-600">{lang === "en" ? "Medical record number" : "病歷號碼"}<input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm font-normal" value={adminDetailBooking.medicalRecordNumber || ""} onChange={(e) => setAdminDetailBooking({ ...adminDetailBooking, medicalRecordNumber: e.target.value })} /></label>
            ...
            <label className="text-xs font-bold text-slate-600">{t.adminChannel}<input className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm font-normal" value={adminDetailBooking.channel || "GENERAL"} onChange={(e) => setAdminDetailBooking({ ...adminDetailBooking, channel: e.target.value })} /></label>
  ...
  const AdminView = () => (
    <div className="min-h-[calc(100vh-73px)] lg:h-[calc(100vh-73px)] min-h-0 max-w-[1400px] mx-auto w-full flex flex-col gap-3 lg:gap-4 p-3 lg:p-6 lg:overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-4 lg:overflow-hidden">
        <div className="flex-1 min-h-0 flex flex-col gap-3 lg:overflow-hidden">
          ...
          <div className="bg-white border border-slate-200 rounded-lg flex-1 min-h-0 flex flex-col overflow-hidden">
            <div className="hidden lg:grid flex-none grid-cols-12 bg-slate-100 text-xs font-bold text-slate-600 px-4 py-2.5 border-b border-slate-200">
              ...
              <div className="col-span-1"><button className="font-bold" onClick={() => toggleAdminSort("package")}>{t.package}{adminSortMark("package")}</button></div>
            ...
            <div className="flex-1 lg:overflow-y-auto divide-y divide-slate-100">
              <div className="lg:hidden grid grid-cols-2 gap-1 border-b border-slate-200 bg-slate-50 p-2 text-xs font-bold text-slate-600">
                <button onClick={() => toggleAdminSort("date")} className="rounded border border-slate-200 bg-white px-2 py-1.5 text-left">{t.appointmentDate}{adminSortMark("date")}</button>
                <button onClick={() => toggleAdminSort("customer")} className="rounded border border-slate-200 bg-white px-2 py-1.5 text-left">{t.customer}{adminSortMark("customer")}</button>
                <button onClick={() => toggleAdminSort("status")} className="rounded border border-slate-200 bg-white px-2 py-1.5 text-left">{t.status}{adminSortMark("status")}</button>
              ...
              {sortedAdminBookings.length ? sortedAdminBookings.map((booking) => (
                <div key={booking.bookingId} onClick={() => setAdminDetailBooking(booking)} className="admin-booking-row grid grid-cols-2 lg:grid-cols-12 items-center gap-x-3 gap-y-2 px-4 py-3 text-sm cursor-pointer hover:bg-slate-50 transition-colors">
                  ...
                  <div className="col-span-1 text-slate-600 truncate"><span className="lg:hidden block text-[10px] text-slate-400">{t.package}</span>{booking.packageName}</div>
        ...
        <div className="w-full lg:w-80 flex-none flex flex-col gap-4 lg:overflow-y-auto">
