...
## 2026-08-12 - IT handoff and architecture documentation

- Rebuilt `docs/CAC_ARCHITECTURE.md` from the current source and production Firebase inventory. The diagrams now match the public `民眾方案` routes and staff `套餐工具`, `預約清單`, `報告管理`, and admin `稽核紀錄` tabs.
- Added `docs/IT_INTEGRATION_GUIDE.md` for hospital IT. It documents the production/staging topology, user instructions, 24 deployed Gen 2 Functions, Firestore access boundaries, LINE/Gmail/Scheduler integrations, deployment and rollback workflow, and unresolved backup/data-retention decisions.
- Defined the pending hospital integration boundaries: OIDC/SAML staff SSO is preferred over collecting AD/LDAP passwords; the cancer-screening API requires a hospital HTTPS gateway, MRN lookup, server-side NHI DLL token, staff-code mapping, and de-identified staging tests.
- Confirmed by Firebase CLI on 2026-08-12 that production Hosting is `channel-activity-customer.web.app` and all 24 deployed Functions use Node.js 22 in `us-central1`.
- Documentation-only change. No application code, Firestore data, rules, Functions, secrets, or deployments changed.