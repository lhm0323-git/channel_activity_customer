...
## 2026-08-12 - Information Services handoff documents

- New primary handoff: `docs/IT_INTEGRATION_GUIDE.md`.
- Updated architecture: `docs/CAC_ARCHITECTURE.md`.
- Both documents distinguish deployed behavior from pending hospital SSO, cancer-screening API, HIS/report integration, corporate one-time claim QR workflow, capacity controls, and formal backup/PITR.
- Current production inventory was verified with Firebase CLI: 24 Node.js 22 Gen 2 Functions in `us-central1`; Hosting site `https://channel-activity-customer.web.app/`.
- Next IT meeting should first resolve: IdP type and metadata, test identities/MFA/disable behavior, hospital HTTPS API gateway, MRN lookup, NHI DLL token boundary, staff-code mapping, retention, logging, and recovery ownership.
- Do not remove the existing bootstrap admin until at least two hospital-domain ADMIN accounts have passed staging login and disabled-account tests.