...
4. **預約操作稽核**
   - 確認、取消、提醒與後台編修一律經受驗證 Cloud Functions 執行。
   - `auditLogs` 僅管理者可查閱，保存操作者、時間與不含個資值的前後差異摘要。
...
- 後台開啟指定預約時，只載入該筆預約的問卷答案；修正儲存後可立即列印簽名紙本。
- 國健署 API 啟用後，資格查詢回傳結構化狀態，且 Firestore 與執行記錄都不含完整身分證。
- 停用員工無法再次進入後台。
- 員工代訂後，客戶可自行以 LINE 綁定該筆預約。
- 民眾只看得到自己的報告狀態，沒有檔案下載連結。
- 管理者可查核預約確認、取消、提醒與編修的操作者、時間與變更欄位。
